...
contract AkuAuction {
    ...
    // TRANSFER FIX (pull payment): the bidder withdraws their own ETH in a SEPARATE call.
    function claimRefund() external {
        uint256 myFinalProcess = finalProcess[msg.sender];
        require(myFinalProcess == 1);
    ...
    function claimProjectFunds() external {
        ...
        require(block.timestamp > expiresAt);
        require(refundProgress >= bidIndex); // STRUCTURAL FIX: compare against bidIndex (unique bidders)
